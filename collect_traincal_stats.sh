...
# 수집 모드 스위치
# 수집기는 4개 feature를 전부 모으지만, 아래 FEAT2 를 명시해두면 로그에서 의도가 드러난다.
export EOCF_EVAL_TRAINCAL_FEAT2=traj_dev
