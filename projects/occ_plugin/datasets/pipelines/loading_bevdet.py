...
class LoadMultiViewImageFromFiles_BEVDet(object):
    ...
    def _load_depth_gt(self, filename, resize, crop, flip, rotate, depth_map_size):
        if not self.depth_gt_path:
            raise ValueError("depth_gt_path must be set when load_depth=True")

        depth_filename = os.path.join(
            self.depth_gt_path,
            f"{os.path.basename(filename)}.bin",
        )
        if not os.path.isfile(depth_filename):
            raise FileNotFoundError(
                f"GT depth file not found: {depth_filename} (image={filename})"
            )

        cam_depth = np.fromfile(depth_filename, dtype=np.float32)
        if cam_depth.ndim != 1:
            raise ValueError(
                f"Malformed GT depth file: expected flat float32 array, got ndim={cam_depth.ndim} "
                f"for {depth_filename}"
            )
        if (cam_depth.size % 3) != 0:
            raise ValueError(
                f"Malformed GT depth file: element count {cam_depth.size} is not divisible by 3 "
                f"for {depth_filename}"
            )

        cam_depth = cam_depth.reshape(-1, 3)
        return depth_transform(
            cam_depth=cam_depth,
            resize=resize,
            resize_dims=depth_map_size,
            crop=crop,
            flip=flip,
            rotate=rotate,
        )
    ...
    def get_seq_inputs(self, results, flip=None, scale=None):
        ...
        for counter in range(sequence_length):
            ...
            for cam_idx, cam_name in enumerate(cam_names):
                ...
                # GT depth loading is intentionally disabled; keep legacy placeholder.
