...
class EfficientOCFDataset(NuScenesDataset):
    def __init__(self, occ_size, pc_range, occ_root, idx_root, ori_data_root, save_local_root, data_root, time_receptive_field, n_future_frames, classes, use_separate_classes,
        ...
        if self.test_mode:
            if self.fixed_val:
                rng = random.Random(self.fixed_val_seed)
                self.chosen_list = rng.sample(range(0, self.test_capacity) , self.test_capacity)
            else:
...
#################################### config 파일의 workers_per_gpu=2,를 위한 설정 ########################
    def __getstate__(self):
        state = self.__dict__.copy()
        # NuScenes detection config carries a dict_keys view and breaks
        # DataLoader worker pickling under the spawn start method.
