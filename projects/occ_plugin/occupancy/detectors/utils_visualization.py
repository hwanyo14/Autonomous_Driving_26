...
class EfficientOCFVisualizationMixin:
    ...
    @torch.no_grad()
    def _maybe_save_lss_pretrain_occ_vis(
        self,
        output_voxels,
        target_voxels: torch.Tensor,
    ) -> None:
        vis_every = int(getattr(self, "pretrain_lss_vis_every", 0))
        if vis_every <= 0:
            return

        step = self._get_pretrain_vis_iteration(advance_if_unsynced=True)
        if (step % vis_every) != 0:
            return
        if not self._is_main_process():
            return

        if isinstance(output_voxels, (list, tuple)):
            if len(output_voxels) <= 0:
                return
            pred_logits = output_voxels[0]
        else:
            pred_logits = output_voxels
        if not torch.is_tensor(pred_logits) or pred_logits.dim() != 4:
            return

        if not torch.is_tensor(target_voxels):
            return
        if target_voxels.dim() == 4:
            gt_btxy = target_voxels.reshape(-1, target_voxels.shape[-2], target_voxels.shape[-1])
        elif target_voxels.dim() == 3:
            gt_btxy = target_voxels
        else:
            return

        bt_pred, c_pred, h_pred, w_pred = pred_logits.shape
        bt_gt = int(gt_btxy.shape[0])
        if bt_gt < bt_pred:
            return
        if bt_gt != bt_pred:
            gt_btxy = gt_btxy[-bt_pred:]

        if c_pred <= 1:
            pred_prob = torch.sigmoid(pred_logits[:, 0].to(torch.float32))
        else:
            pred_prob = torch.softmax(pred_logits.to(torch.float32), dim=1)[:, 1]

        gh, gw = int(gt_btxy.shape[-2]), int(gt_btxy.shape[-1])
        if (h_pred != gh) or (w_pred != gw):
        ...
        gt = gt_btxy.to(torch.long)
        gt_ignore = (gt == 255)
        gt_occ = (gt > 0) & (~gt_ignore)
        pred_prob = pred_prob.clamp(0.0, 1.0)
        pred_bin = pred_prob >= float(self.pretrain_lss_vis_prob_thr)

        max_frames = max(1, int(getattr(self, "pretrain_lss_vis_max_frames", 6)))
        n_show = min(max_frames, bt_pred)
        if n_show <= 0:
            return
        ...
        row_h = gh
        row_w = gw
        canvas_w = n_show * row_w + (n_show - 1) * gap
        ...
        for i in range(n_show):
            ...
            gt_rgb = np.zeros((gh, gw, 3), dtype=np.uint8)
            gt_occ_i = gt_occ[i].cpu().numpy()
            gt_ign_i = gt_ignore[i].cpu().numpy()
            gt_rgb[gt_occ_i] = np.array([240, 240, 240], dtype=np.uint8)
            gt_rgb[gt_ign_i] = np.array([100, 100, 100], dtype=np.uint8)

            prob_i = pred_prob[i].cpu().numpy()
            prob_rgb = np.zeros((gh, gw, 3), dtype=np.uint8)
            prob_rgb[..., 0] = np.clip(prob_i * 255.0, 0.0, 255.0).astype(np.uint8)

            ov_rgb = np.zeros((gh, gw, 3), dtype=np.uint8)
            pred_i = pred_bin[i].cpu().numpy()
            ov_rgb[gt_occ_i] = np.array([40, 180, 40], dtype=np.uint8)
            ov_rgb[pred_i] = np.array([220, 60, 60], dtype=np.uint8)
            ov_rgb[gt_occ_i & pred_i] = np.array([255, 220, 70], dtype=np.uint8)
            ov_rgb[gt_ign_i] = np.array([100, 100, 100], dtype=np.uint8)

            y0 = text_h
            y1 = y0 + row_h + gap
            y2 = y1 + row_h + gap
            canvas[y0:y0 + row_h, x0:x0 + row_w] = gt_rgb
            canvas[y1:y1 + row_h, x0:x0 + row_w] = prob_rgb
            canvas[y2:y2 + row_h, x0:x0 + row_w] = ov_rgb

        img = Image.fromarray(canvas, mode="RGB")
        draw = ImageDraw.Draw(img)
        draw.text(
            (2, 1),
            "row1: GT occ(white)/ignore(gray)  row2: pred occ prob(red)  row3: overlay(green=gt, red=pred, yellow=overlap)",
            fill=(255, 255, 255),
        )
        for i in range(n_show):
            x0 = i * (row_w + gap)
            gt_ratio = float(gt_occ[i].float().mean().item())
            pred_ratio = float(pred_bin[i].float().mean().item())
            draw.text((x0 + 2, text_h + 2), f"idx={i}", fill=(255, 255, 255))
            draw.text((x0 + 2, text_h + row_h + gap + 2), f"p_mean={float(pred_prob[i].mean().item()):.3f}", fill=(255, 255, 255))
            draw.text((x0 + 2, text_h + (row_h + gap) * 2 + 2), f"gt={gt_ratio:.3f} pred={pred_ratio:.3f}", fill=(255, 255, 255))

        vis_dir = str(self.pretrain_lss_vis_dir)
        os.makedirs(vis_dir, exist_ok=True)
        out_path = os.path.join(vis_dir, f"iter_{step:06d}_lss_bev_occ.png")
        img.save(out_path)
