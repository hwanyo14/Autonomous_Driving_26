...
class EfficientOCFLossMixin:
    ...
    def _compute_query_feat_align_loss(
        self,
        ...
        """
        Matched-only feature alignment on context-seq pooled image features.
        Query feature: transformer attention pooled feature (`query_img_feat_pooled`).
        GT feature: instance-wise pooled feature from `context_seq`.
        """
        ...
        out = {
            "loss_query_feat_align": z,
            "dbg_query_feat_align_matched": z,
            "dbg_query_feat_align_unmatched_neg": z,
            "dbg_query_unmatched_count": z.new_tensor(0.0),
            "dbg_query_unmatched_max_sim_mean": z.new_tensor(0.0),
            "sim_tqn": None,
            "sim_best_tq": None,
            "sim_ratio_tq": None,
            "query_bev_cos_tq": None,
            "feat_weight_tq": None,
            "valid_query_tq": None,
        }
        matched_pull_done = False
        if isinstance(inst_match_result, dict):
            matched_query_feat_tkd = inst_match_result.get("matched_query_feat_tkd", None)
            matched_gt_feat_tkd = inst_match_result.get("matched_gt_feat_tkd", None)
            matched_gt_feat_valid_tk = inst_match_result.get("matched_gt_feat_valid_tk", None)
            if (
                torch.is_tensor(matched_query_feat_tkd)
                and torch.is_tensor(matched_gt_feat_tkd)
                and matched_query_feat_tkd.dim() == 3
                and matched_gt_feat_tkd.dim() == 3
                and tuple(matched_query_feat_tkd.shape) == tuple(matched_gt_feat_tkd.shape)
                and matched_query_feat_tkd.numel() > 0
            ):
                q_sel_tkd = matched_query_feat_tkd.to(torch.float32)
                g_sel_tkd = matched_gt_feat_tkd.to(device=q_sel_tkd.device, dtype=torch.float32)
                valid_tk = torch.ones(
                    q_sel_tkd.shape[:2], device=q_sel_tkd.device, dtype=torch.bool
                )
                if (
                    torch.is_tensor(matched_gt_feat_valid_tk)
                    and matched_gt_feat_valid_tk.dim() == 2
                    and tuple(matched_gt_feat_valid_tk.shape) == tuple(valid_tk.shape)
                ):
                    valid_tk = valid_tk & matched_gt_feat_valid_tk.to(device=q_sel_tkd.device, dtype=torch.bool)
                valid_f = valid_tk.to(torch.float32)
                valid_cnt = valid_f.sum()
                if bool((valid_cnt > 0).item()):
                    qn = F.normalize(q_sel_tkd, dim=-1, eps=1e-6)
                    gn = F.normalize(g_sel_tkd, dim=-1, eps=1e-6)
                    cos_tk = F.cosine_similarity(qn, gn, dim=-1, eps=1e-6).clamp(-1.0, 1.0)
                    feat_dist_tk = 0.5 * (1.0 - cos_tk)
                    loss_matched_raw = (feat_dist_tk * valid_f).sum() / valid_cnt.clamp_min(1.0)
                    out["dbg_query_feat_align_matched"] = (
                        loss_matched_raw * float(self.query_feat_align_loss_weight)
                    )
                    out["query_bev_cos_tq"] = cos_tk
                    out["feat_weight_tq"] = valid_f
                    out["valid_query_tq"] = valid_tk
                    matched_pull_done = True

        if (not torch.is_tensor(query_future_feat_tqd)) or query_future_feat_tqd.dim() != 3:
            out["loss_query_feat_align"] = out["dbg_query_feat_align_matched"]
            return out
        if (not torch.is_tensor(gt_inst_bev_feat_tnd)) or gt_inst_bev_feat_tnd.dim() != 3:
            out["loss_query_feat_align"] = out["dbg_query_feat_align_matched"]
            return out

        Tq, Q, Dq = [int(v) for v in query_future_feat_tqd.shape]
        Tg, Ng, Dg = [int(v) for v in gt_inst_bev_feat_tnd.shape]
        if Dq != Dg:
            out["loss_query_feat_align"] = out["dbg_query_feat_align_matched"]
            return out
        T = min(Tq, Tg)
        ...
        g_feat_tnd = gt_inst_bev_feat_tnd[:T].to(device=query_future_feat_tqd.device, dtype=torch.float32)
        valid_gt_tn = torch.ones((T, Ng), dtype=torch.bool, device=query_future_feat_tqd.device)
        if (
            torch.is_tensor(gt_inst_bev_feat_valid_tn)
            and gt_inst_bev_feat_valid_tn.dim() == 2
            and int(gt_inst_bev_feat_valid_tn.shape[0]) >= T
            and int(gt_inst_bev_feat_valid_tn.shape[1]) == Ng
        ):
            valid_gt_tn = valid_gt_tn & gt_inst_bev_feat_valid_tn[:T].to(
                device=query_future_feat_tqd.device, dtype=torch.bool
            )
        gt_center_valid_tn = None
        if isinstance(inst_match_result, dict):
            gt_center_valid_tn = inst_match_result.get("gt_valid_tn", None)
        if (
            torch.is_tensor(gt_center_valid_tn)
            and gt_center_valid_tn.dim() == 2
            and int(gt_center_valid_tn.shape[0]) >= T
            and int(gt_center_valid_tn.shape[1]) == Ng
        ):
            valid_gt_tn = valid_gt_tn & gt_center_valid_tn[:T].to(
                device=query_future_feat_tqd.device, dtype=torch.bool
            )

        matched_query_idx = None
        matched_inst_idx = None
        matched_q_idx = None
        matched_i_idx = None
        if isinstance(inst_match_result, dict):
            matched_query_idx = inst_match_result.get("matched_query_idx", None)
            matched_inst_idx = inst_match_result.get("matched_inst_idx", None)
        if torch.is_tensor(matched_query_idx) and torch.is_tensor(matched_inst_idx):
            if matched_query_idx.numel() > 0 and matched_query_idx.numel() == matched_inst_idx.numel():
                mq = matched_query_idx.to(device=query_future_feat_tqd.device, dtype=torch.long)
                mi = matched_inst_idx.to(device=query_future_feat_tqd.device, dtype=torch.long)
                keep = (mq >= 0) & (mq < Q) & (mi >= 0) & (mi < Ng)
                if bool(keep.any().item()):
                    mq = mq[keep]
                    mi = mi[keep]
                    if mq.numel() > 0:
        ...
        # 1) Matched-pair pull loss
        if (matched_q_idx is not None) and (matched_i_idx is not None) and (not matched_pull_done):
            q_sel_tkd = q_feat_tqd.index_select(1, matched_q_idx)
            g_sel_tkd = g_feat_tnd.index_select(1, matched_i_idx)
            valid_tk = valid_gt_tn.index_select(1, matched_i_idx)
    ...
    def _compute_query_cam_proj_consistency_loss(
        self,
        pred_proj_tknhw: torch.Tensor,
        pred_valid_tkn: torch.Tensor,
        gt_bbox_tknhw: torch.Tensor,
        gt_bbox_valid_tkn: torch.Tensor,
        metric: str = "bce",
        pred_norm: str = "amax",
        eps: float = 1e-6,
        weight: float = 1.0,
    ) -> dict:
        if torch.is_tensor(pred_proj_tknhw):
            z = pred_proj_tknhw.sum() * 0.0
        elif torch.is_tensor(gt_bbox_tknhw):
            z = gt_bbox_tknhw.sum() * 0.0
        else:
            z = self.mean_weight.sum() * 0.0
        ...
        metric_code = {
            ...
            "bce_dice": 2.0,
            "kl": 3.0,
        }.get(str(metric).lower(), -1.0)
        pred_norm_code = {
            "amax": 0.0,
            "sum": 1.0,
            "none": 2.0,
        }.get(str(pred_norm).lower(), -1.0)
        out = {
            "loss_query_cam_proj_consistency": z,
            "dbg_query_cam_proj_consistency_valid_pair_count": z,
            "dbg_query_cam_proj_consistency_raw_mean": z,
            "dbg_query_cam_proj_consistency_shape_invalid_skip": z,
            "dbg_query_cam_proj_consistency_nonfinite_skip": z,
            "dbg_query_cam_proj_consistency_metric": z.new_tensor(float(metric_code)),
            "dbg_query_cam_proj_consistency_pred_norm": z.new_tensor(float(pred_norm_code)),
        }
        if (
            (not torch.is_tensor(pred_proj_tknhw))
            or (not torch.is_tensor(pred_valid_tkn))
            or (not torch.is_tensor(gt_bbox_tknhw))
            or (not torch.is_tensor(gt_bbox_valid_tkn))
            or pred_proj_tknhw.dim() != 5
            or pred_valid_tkn.dim() != 3
            or gt_bbox_tknhw.dim() != 5
            or gt_bbox_valid_tkn.dim() != 3
        ):
            out["dbg_query_cam_proj_consistency_shape_invalid_skip"] = z.new_tensor(1.0)
            return out

        if tuple(pred_proj_tknhw.shape) != tuple(gt_bbox_tknhw.shape):
            out["dbg_query_cam_proj_consistency_shape_invalid_skip"] = z.new_tensor(1.0)
            return out
        if tuple(pred_valid_tkn.shape) != tuple(gt_bbox_valid_tkn.shape):
            out["dbg_query_cam_proj_consistency_shape_invalid_skip"] = z.new_tensor(1.0)
            return out
        if tuple(pred_valid_tkn.shape) != tuple(pred_proj_tknhw.shape[:3]):
            out["dbg_query_cam_proj_consistency_shape_invalid_skip"] = z.new_tensor(1.0)
            return out

        metric_v = str(metric).lower()
        if metric_v not in ("bce", "dice", "bce_dice", "kl"):
            out["dbg_query_cam_proj_consistency_shape_invalid_skip"] = z.new_tensor(1.0)
            return out
        norm_v = str(pred_norm).lower()
        if norm_v not in ("amax", "sum", "none"):
            out["dbg_query_cam_proj_consistency_shape_invalid_skip"] = z.new_tensor(1.0)
            return out

        eps_v = float(max(1e-12, eps))
        pred = pred_proj_tknhw.to(torch.float32).clamp_min(0.0)
        tgt = gt_bbox_tknhw.to(device=pred.device, dtype=torch.float32).clamp(0.0, 1.0)
        valid_tkn = pred_valid_tkn.to(device=pred.device, dtype=torch.bool) & gt_bbox_valid_tkn.to(
            device=pred.device, dtype=torch.bool
        )

        T, K, N, H, W = [int(v) for v in pred.shape]
        P = int(H * W)
        pred_tknp = pred.reshape(T, K, N, P)
        tgt_tknp = tgt.reshape(T, K, N, P)
        valid_tkn = valid_tkn & torch.isfinite(pred_tknp).all(dim=-1) & torch.isfinite(tgt_tknp).all(dim=-1)

        if norm_v == "amax":
            den_tkn1 = pred_tknp.amax(dim=-1, keepdim=True)
            valid_tkn = valid_tkn & (den_tkn1[..., 0] > 0.0)
            pred_metric = pred_tknp / den_tkn1.clamp_min(eps_v)
        elif norm_v == "sum":
            den_tkn1 = pred_tknp.sum(dim=-1, keepdim=True)
            valid_tkn = valid_tkn & (den_tkn1[..., 0] > 0.0)
            pred_metric = pred_tknp / den_tkn1.clamp_min(eps_v)
        else:
            pred_metric = pred_tknp

        dist_tkn = torch.zeros((T, K, N), device=pred.device, dtype=torch.float32)
        if metric_v == "kl":
            pred_kl_sum = pred_metric.sum(dim=-1, keepdim=True)
            tgt_kl_sum = tgt_tknp.sum(dim=-1, keepdim=True)
            valid_tkn = valid_tkn & (pred_kl_sum[..., 0] > 0.0) & (tgt_kl_sum[..., 0] > 0.0)
            pred_kl = pred_metric / pred_kl_sum.clamp_min(eps_v)
            tgt_kl = tgt_tknp / tgt_kl_sum.clamp_min(eps_v)
            finite_tkn = torch.isfinite(pred_kl).all(dim=-1) & torch.isfinite(tgt_kl).all(dim=-1)
            valid_tkn = valid_tkn & finite_tkn
            log_pred = torch.log(pred_kl.clamp_min(eps_v))
            log_tgt = torch.log(tgt_kl.clamp_min(eps_v))
            dist_tkn = (tgt_kl * (log_tgt - log_pred)).sum(dim=-1)
        else:
            ...
            if metric_v in ("dice", "bce_dice"):
                inter_tkn = (pred_metric * tgt_tknp).sum(dim=-1)
                denom_tkn = pred_metric.sum(dim=-1) + tgt_tknp.sum(dim=-1)
                dice_tkn = (2.0 * inter_tkn + eps_v) / (denom_tkn + eps_v)
                dice_dist_tkn = 1.0 - dice_tkn
            else:
                dice_dist_tkn = torch.zeros_like(dist_tkn)

            if metric_v == "bce":
                dist_tkn = bce_tkn
            elif metric_v == "dice":
                dist_tkn = dice_dist_tkn
            else:
                dist_tkn = 0.5 * (bce_tkn + dice_dist_tkn)

        finite_dist = torch.isfinite(dist_tkn)
        nonfinite_skip = (valid_tkn & (~finite_dist)).to(torch.float32).sum()
        valid_tkn = valid_tkn & finite_dist
        valid_f = valid_tkn.to(torch.float32)
        valid_count = valid_f.sum()
        out["dbg_query_cam_proj_consistency_nonfinite_skip"] = nonfinite_skip
        out["dbg_query_cam_proj_consistency_valid_pair_count"] = valid_count
        if not bool((valid_count > 0).item()):
            return out

        raw_mean = (dist_tkn * valid_f).sum() / valid_count.clamp_min(1.0)
        out["dbg_query_cam_proj_consistency_raw_mean"] = raw_mean
        out["loss_query_cam_proj_consistency"] = raw_mean * float(weight)
        return out
