...
class DepthNet(nn.Module):
    ...
    def prepare_inputs(self, x, mlp_input):
        mlp_input = self.bn(mlp_input.reshape(-1, mlp_input.shape[-1]))
        x = self.reduce_conv(x)
        return x, mlp_input

    def forward_depth_from_prepared(self, x, mlp_input):
        depth_se = self.depth_mlp(mlp_input)[..., None, None]
        depth = self.depth_se(x, depth_se)
        depth = self.depth_conv(depth)
        return depth

    def forward_context_from_prepared(self, x, mlp_input):
    ...
    def forward(self, x, mlp_input):
        x, mlp_input = self.prepare_inputs(x, mlp_input)
        context = self.forward_context_from_prepared(x, mlp_input)
        depth = self.forward_depth_from_prepared(x, mlp_input)
...
class ViewTransformerLSSBEVDepth(ViewTransformerLiftSplatShoot):
    ...
    def forward(self, input):
        ...
        B, N, C, H, W = x.shape
        depth_digit = input[8] if len(input) > 8 else None
        img_feat = input[9] if len(input) > 9 else None
