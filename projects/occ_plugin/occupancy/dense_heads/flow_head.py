...
class FlowHead(nn.Module):
    def __init__(
        ...
        self.last_conv.bias = nn.parameter.Parameter(torch.tensor([0.0, 0.0], requires_grad=True))
    ...
    def loss_voxel(self, output_voxels, target_voxels, tag):   
        ...
        tB, tC, tF, tH, tW = target_voxels.shape
        target_voxels = target_voxels.view(tB*tC, tF, tH, tW)
        
        assert torch.isnan(output_voxels).sum().item() == 0

        output_voxels = output_voxels.unsqueeze(-1).permute(0,2,3,4,1)
        target_voxels = target_voxels.unsqueeze(-1).permute(0,2,3,4,1)

        loss_dict = {}

        loss_dict['loss_flow_l1_{}'.format(tag)] = (0.01) * Smooth_L1_loss(output_voxels, target_voxels, ignore_index=255)
