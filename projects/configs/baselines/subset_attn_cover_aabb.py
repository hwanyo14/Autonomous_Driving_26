...
#
# [_aabb 실험 2026-07-06] subset_attn_cover 위에 focal3d GT 혼합 추가 — 단일변수 =
#   query_gmo_focal_inst3d_weight(0.1)/query_gmo_focal_bbox_weight(0.9) + gt_bbox_aabb 로드.
# 요지: matched-pair focal(loss_gmo_focal)의 GT를 pair별로
#   0.1×gt_occ_inst3d(실점유 voxel) + 0.9×AABB bbox(annotation box 채움)로 가중합.
#   dice(tversky)·매칭·center는 기존대로 inst3d만 사용 (utils_loss.py 혼합 주석 참조).
# bbox GT = data/efficientocf_bboxcls_v3 (train 23,930 전체; gen_bbox_gt_v2.py --split train
#   --aabb_only --include_ped_ids). inst id는 gt_occ_inst와 동일한 ped-포함 번호 체계 —
#   v2(val용)는 ped-제외 번호라 id가 어긋나므로 학습 loss에 쓰면 안 됨 (NOTES 2026-07-06).
#   ped 행(cls=7)은 로더 exclude_occ_class_ids가 제거, 번호만 유지됨.
# 모니터: dbg_gmo_focal_inst3d vs dbg_gmo_focal_bbox (pair 평균), dbg_gmo_focal_bbox_pair_count.
#
...
model_cfg = dict(
    ...
    # [_aabb 실험] focal GT 혼합 0.1 inst3d + 0.9 AABB bbox — 파일 상단 주석 참조.
    query_gmo_focal_inst3d_weight=0.1,
    query_gmo_focal_bbox_weight=0.9,
