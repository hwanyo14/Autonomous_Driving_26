...
model_cfg = dict(
    ...
    # ===== [per-query instance scale] 큰 객체(버스/트레일러) 커버 — forcing-only ===========
    # 천장(budget) 커플링·scale head/Lreg는 OFF → offset/sigma 캡은 위 전역 고정값(기존) 그대로.
    # forcing(one-sided spread 정칙항)만 사용: matched query의 effective 2nd-moment 반경을
    #   0.5·GT_half_extent까지 끌어올림(부족할 때만) → 큰 객체를 GT extent까지 펴고,
    #   차/이미 큰 건 영향 0, over-spread는 Tversky FP가 캡. GT extent = voxel-AABB(max-over-frames).
