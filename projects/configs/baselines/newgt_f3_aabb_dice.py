# ============================================================================
# newgt_f3_aabb_dice — 학습 주력 config
#   데이터  : ./data/efficientocf_gt_f3/  (생성소멸 필터판: 관측창 0~2 교집합, 사람 포함)
#   사람제거: 로더 exclude_occ_class_ids=(7,) — CW 포함 cls=7 전부 로드 시 제거
#             (eval은 efficientocf.py 내장 (gt!=7) 마스크가 담당, 시각화는 동일 마스크 적용)
#   raw와 완벽 호환: 포맷/키/id 동일, 생성소멸 인스턴스만 빠짐 — 경로만 바꾸면 raw로 복귀
#   eval 시: EOCF_BBOX_GT_V2_DIR=./data/efficientocf_gt_f3/GMO
# ============================================================================

# ============================================================================
# newgt_aabb_dice — 새 GT 파이프라인(efficientocf_gt) 배선 검증용 config
# base: subset_attn_cover_aabb_dice.py, 변경점:
#   - gt_occ_inst(D)·gt_bbox_aabb(E) 경로 → ./data/efficientocf_gt/ (단일 캐시 루트)
#   - load_segmentation_instance3d/cls_instance3d = False
#     → center/attn GT는 로더의 §8.3 분기(E에서 생성), cls GT는 detector의
#       gt_occ_inst 번들 파생(seg_cls_inst_tcxzy 우선) 사용 — 구 캐시 완전 배제
#   - exclude_occ_class_ids = () — raw GT 그대로 (사람제거/생성소멸 필터는 추후
#     데이터 단계에서 별도 수행 예정, 로더 필터 없음)
#   - test pipeline에도 load_gt_bbox_aabb=True (val에서도 E 기반 center GT)
# 주의: eval rot coverage는 EOCF_BBOX_GT_V2_DIR=./data/efficientocf_gt/GMO 로 지정.
# ============================================================================
...
exclude_occ_class_ids = (7,)  # 사람 제거는 로더에서 (데이터는 사람 포함 f3 — 끄려면 ()로)
