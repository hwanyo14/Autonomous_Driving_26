...
#   + query_transformer_kv_resolutions — 마지막 해상도는 이 레포 img_neck(SECONDFPN, stride16
#   통합)의 실제 원본 context feature 해상도(= input_size/16)와 반드시 일치해야 함
#   (transformer.py:236 ValueError로 하드 검증). 이 _res704 변형은 (16,44).
...
#
# [_res704 실험 2026-08-03] _feat128(bev_feat_dim=128) 대비 입력 해상도만 BEVDet 표준
#   (896,1600) → (256,704)로 낮춘 변형. 바뀌는 값은 3개:
#     1) data_config['input_size'] (896,1600) → (256,704)
#     2) query_transformer_kv_resolutions ((14,25),(28,50),(56,100)) → ((4,11),(8,22),(16,44))
#        — 1)의 종속 변수. img_neck stride16이므로 마지막 layer = input_size/16 = (16,44).
#          피라미드는 16→8→4 / 44→22→11 정수배라 transformer.py의 avg_pool2d 정확 경로 유지
#          (adaptive_avg_pool2d fallback 안 탐). learnable_kv_downsample=False라 신규 파라미터 없음.
#     3) query_attn_sigma_match_loss_weight 0.25 → 0.0 (_cover 축 의도적 off)
#   파생 효과: 카메라당 입력 픽셀·context 토큰·KV 토큰·LSS frustum ray·depth loss 격자가
#     전부 정확히 1/8로 감소 (모두 stride16 하나에 묶여 있음). KV 총합 44,100 → 5,544.
#   ※ 숨은 두 번째 변수 — crop이 같이 바뀐다 (loading_bevdet.py sample_augmentation):
#     base resize = fW/1600이 1.0 → 0.44, newH 900 → 396, crop_h 4px → 140px.
#     즉 세로 FOV 유지율 99.6% → 64.6%로 상단 약 35%가 잘린다 (가까운 트럭/버스 상단 clip).
#     BEVDet 256×704 표준 레시피 그대로지만 "단순 downscale"이 아님을 판정 시 감안할 것.
#   ※ σ-match를 꺼도 나머지 attn 감독(use_query_attn_bbox_loss / other 0.3 / inside_log cost)은
#     그대로 켜져 있고, 이제 카메라당 (16,44)=704칸 격자에서 계산된다 — 원거리 객체가 1칸 미만이
#     되므로 inside-mass/soft-IoU 신호가 거칠어지는 점이 이 실험의 주 리스크.
#   불변: occ_size/point_cloud_range/grid_config/bev_feat_dim(128)/query_num_queries/
#     query_matched_gmo_bce_occ_size/gaussian·dice·tversky·traj 전부. BEV·loss 격자는 그대로이고
#     그 격자를 채우는 이미지 증거의 밀도만 1/8이 된다.
#   파라미터 shape은 해상도 무관(FCN + sin-cos pos embed 매 forward 생성) → ckpt shape 호환.
#   visualization_cfg의 4개 경로는 학습에선 mmdet_train.py _configure_visualization_dirs가
#     {work_dir}/vis/{timestamp}/로, eval에선 EOCF_EVAL_VIS_DIR로 덮어쓰므로 실제로는 미사용값.
...
data_config = {
    ...
    'Ncams': 6,
    # [_res704] (896,1600) → (256,704). (H, W) 순서. src_size는 원본 이미지 크기라 불변.
    # 종속: query_transformer_kv_resolutions 마지막 layer = input_size/16 = (16,44).
    'input_size': (256, 704),
    'src_size': (900, 1600),
...
train_pipeline = [
    ...
    dict(
        ...
        test_mode=False,
        # ./data/occ_dt가 이 머신에 없고 use_query_dt_loss=False라 occ_dt는 아무 데도 쓰이지
        # 않는다. True로 두면 LoadOccupancy가 첫 샘플에서 FileNotFoundError로 죽는다.
        dt_path=occ_dt_path,
        load_occ_dt=False,
        dt_type='float16',
        strict_dt=True,
...
model_cfg = dict(
    ...
    # [_res704] 0.25 → 0.0 으로 의도적 off. res704에서는 GT cam 마스크 σ가 픽셀 기준으로
    #   ~2.3배 작아져 폭 회귀 타깃 자체가 불안정해지므로 이 축을 빼고 해상도만 본다.
    #   weight=0이면 loss 항이 생성되지 않아 아래 start_iter/min_mask_px도 함께 무효화된다
    #   (되살릴 때 두 값도 res704 픽셀 스케일로 재검토할 것 — min_mask_px=4는 896 기준).
    ...
    # (256,704) 기준 context feature 원본 해상도가 (16,44)이므로 최종 layer를 원본 해상도로 맞춤.
    # layer1(4,11) 전역 대략 위치 -> layer2(8,22) 중간 정제 -> layer3(16,44) 원본 해상도 정밀화.
    # [_res704] ((14,25),(28,50),(56,100)) → ((4,11),(8,22),(16,44)). input_size의 종속 변수이며
    #   마지막 값이 실제 context 해상도와 다르면 transformer.py:236에서 ValueError로 죽는다.
