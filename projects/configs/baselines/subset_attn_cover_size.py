...
#
# [_size 실험 2026-07-05] cover 위에 size note("크기 쪽지") 추가 — cover 대비 단일변수
# = query_size_note_enabled/query_size_aux_loss_weight/big_weight_cap 3키 + Collect에
# gt_instance_sizes/gt_instance_dims. 목적 = 큰 객체 shape: gaussian head가 "이 query가
# 몇 m짜리인지"를 입력으로 받게 함 (aux size head 예측 + attn 폭 σ + depth). 벌리는 힘은
# 이번엔 기존 matched-GMO dice에 맡김 (렌더-recall loss는 사용자 결정으로 후속 스테이지).
# 판정: ① dbg_query_size_aux_l1_mean/big 수렴 (특히 big — feature의 크기 학습 가능성),
#   ② ep5 tools/dbg_probe/vis_attn_by_size.py로 버스 BEV가 넓어지는지 (before:
#   work_dirs/subset_attn_cover/dbg_analysis/attn_vis_by_size.png), ③ ep15 eval Recall3d.
# 상세 배경: NOTES.md 2026-07-02~05, CHANGELOG.md 2026-07-05.
...
model_cfg = dict(
    ...
    # ===== [_size 실험 2026-07-05] size note ("크기 쪽지") — cover 대비 단일변수 =====
    # gaussian head가 객체 크기를 알고 뿌리게: [logσu, logσv, log d, log w_pred, log l_pred]
    # 5성분을 zero-init 투영으로 head 입력에 additive 주입 (전부 detach — attention/depth/
    # aux 역류 차단). σ=attn 폭(cover가 크기 추종하게 만든 그 값), d=center 거리,
    # w/l=aux size head 예측.
    # 근거: ep10/15 probe 실측 — 크기 정보가 모델 어디에도 없어 모든 객체가 car-size로
    #   찍힘(NOTES 2026-07-02~04). naive σ×d는 실패(시야각 단축+폭 바닥, spearman -0.05)
    #   → 성분을 날것으로 주고 MLP가 보정을 학습하는 형태.
    # 주의: head 파라미터가 늘어나 기존 ckpt와 비호환 — from-scratch 전용.
    query_size_note_enabled=True,
    # aux size head: matched query의 (w,l)을 annotation 원본 치수(gt_instance_dims,
    # 회전 무관 진짜 크기)로 L1 회귀. dbg_query_size_aux_l1_big(>6m 대형 bin)이
    # "feature가 크기를 배울 수 있는가"의 상시 지표 — 이게 안 내려가면 기하 성분만 유효.
    query_size_aux_loss_weight=0.5,
    # 크기-비례 가중 상한: weight=clamp(max(w,l)/2m, 1, cap). 버스(l~12m)=4배.
