...
model_cfg = dict(
    ...
    query_num_classes=2,
    # [q100 ablation] bg weight를 query 수에 반비례로 스케일: 0.05 × (200/100) = 0.1.
    # cls loss는 weighted CE reduction='mean' (Σ w_y·l / Σ w_y) 이라, bg weight를 고정하면
    # unmatched query가 절반으로 줄면서 (a) bg/fg 질량비와 (b) 정규화 분모(=fg per-instance
    ...
    query_num_queries=100,  # [q100 ablation] main run(200) 대비 query 수만 1/2
...
visualization_cfg = dict(
    # ── 2D query_debug_vis ─────────────────────────────────────────────
    debug_query_vis_dir="./work_dirs/subset_attn_cover_pyr_aabb_dice3d_new_asset_all_filter_q100/query_debug_vis",
    debug_query_gaussian_vis_mode='prob',     # footprint 렌더 외형(prob heatmap)
    # ── cam_gaussian ───────────────────────────────────────────────────
    debug_query_cam_gaussian_vis_dir="./work_dirs/subset_attn_cover_pyr_aabb_dice3d_new_asset_all_filter_q100/query_cam_gaussian_vis",
    ...
    # ── 3D mixture3d ───────────────────────────────────────────────────
    debug_query_mixture3d_vis_dir="./work_dirs/subset_attn_cover_pyr_aabb_dice3d_new_asset_all_filter_q100/query_mixture3d_vis",
    ...
    debug_query_attn_softargmax_vis_dir=(
        "./work_dirs/subset_attn_cover_pyr_aabb_dice3d_new_asset_all_filter_q100/query_attn_softargmax_vis"
    ),
)
