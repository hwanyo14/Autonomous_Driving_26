...
→ `traincal/res704_ep18_n4300.json` 은 의도한 4,000 이 아니다. **재수집하지 않기로 함**
...
> ⚠ **2026-08-10 폐기.** filter_eval 이식(CHANGELOG 동일 날짜)으로 refine 적용 지점이
> 단일화되고 **기본값이 ON** 으로 바뀌었다. 이제 끄려면 `EOCF_EVAL_TRAJ_REFINE=0` 을 준다.
> 아래 내용은 이식 이전 상태 기록이다.
...
- refine head(`query_traj_xy_refine_*`)는 학습 loss 경로에서만 궤적을 보정해 왔다. 따라서
  ...
  결과는 refine 없는 궤적이다. refine 관련 결과 해석 전에 이 스위치 값을 먼저 확인할 것.
- 학습 중에도 `query_traj_xy_refine_loss_weight > 0.0`일 때만 refine이 돈다. warmup 스케줄상
  초반 epoch은 weight 0.0이라 그 구간엔 refine head가 아예 학습되지 않는다 — 그 시점
  체크포인트로 스위치를 켜면 미학습 head가 궤적을 흔든다.
- eval 분기는 `not self.training` 게이트가 필수다. `extract_feat_query`는 train/test 공용이라
  게이트를 빼면 forward_train의 refine과 이중 적용된다.
- `eval_total.sh`와 `eval_oracle.sh`의 값이 서로 다르면 oracle 진단이 무효다.
