...
# [ablation] q100 — query_num_queries 200 -> 100
#   main run(feat128, 200) 대비 query 수만 1/2. 코드 변경 없이 config 파라미터만 다르다.
#   checkpoint 는 ablation_query 레포에 있고 config 만 이 레포에 있다.
#   ⚠ 이 run 은 epoch 6 까지만 학습돼 있다 (q900/center 는 15).
#     ep15 끼리 비교하는 표에 ep6 값을 섞지 말 것.

# ----- 대상 config / checkpoint / GPU -----
CONFIG=./projects/configs/baselines/full_attn_cover_pyr_aabb_dice3d_new_asset_all_filter_q100_feat128.py
CHECKPOINT=/home/hwanhee/Autonomous_Driving_26_filter_ablation_query/work_dirs/full_attn_cover_pyr_aabb_dice3d_new_asset_all_filter_q100_feat128/epoch_6_lss_only.pth
GPUS=1
export PORT=50712
...
[ -f "$CHECKPOINT" ] || { echo "[eval_q100] checkpoint 없음: $CHECKPOINT"; exit 1; }
echo "[eval_q100] ckpt=$(basename "$CHECKPOINT") occ=$EOCF_EVAL_OCC_THR fg=$EOCF_EVAL_FG_THR maxs=$EOCF_EVAL_MAX_SAMPLES"
