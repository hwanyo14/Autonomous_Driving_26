...
# ⚠ config/checkpoint 는 baseline(eval_total.sh)과 **반드시 동일**해야 한다.
#   다르면 oracle vs baseline 비교가 "선택 방식" 차이가 아니라 모델 차이까지 섞인다.
CONFIG=./projects/configs/baselines/full_attn_cover_pyr_aabb_dice3d_new_asset_all_filter_feat128.py
CHECKPOINT=./work_dirs/full_attn_cover_pyr_aabb_dice3d_new_asset_all_filter_feat128/epoch_17_lss_only.pth
GPUS=1
export PORT=50091
...
export EOCF_EVAL_MAX_SAMPLES=0
