"""feat128 traincal 정규화 통계 수집용 config.

`_feat128` 을 그대로 상속하고 **`data.test.ann_file` 만 train 으로 바꾼다**.
...
사용(권장): 이 디렉터리의 통계 수집 래퍼를 쓴다.
    CFG_NAME=<베이스 config명> EP=<에폭> OUT=./traincal/<이름>.json MAXS=4000 \
        bash collect_traincal_stats.sh
직접 호출:
...
⚠ checkpoint 종속이다. feat128 ep19 통계를 dim96 이나 다른 에폭에 재사용하면
  z-score 가 통째로 틀어지는데 **에러가 나지 않는다**(NOTES 2026-08-03).
"""

_base_ = ['../projects/configs/baselines/full_attn_cover_pyr_aabb_dice3d_new_asset_all_filter_feat128.py']
...
data = dict(
    test=dict(
        ...
        # train split 전체를 대상으로 둔다. 실제 수집량은
        # EOCF_EVAL_MAX_SAMPLES 로 조절하고, 중간 스냅샷이 계속 저장되므로
        # 4,000 지점에서 꺼내 쓰고 그대로 더 돌려도 된다.
        test_capacity=23930,
    ),
)
